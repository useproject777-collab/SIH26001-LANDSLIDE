...
from database import SessionLocal, initialize_database  # noqa: E402
...
def main():
    ...
    try:
        ...
        app_users = db.execute(text("SELECT to_regclass('public.app_users')")).scalar()
        email_otps = db.execute(text("SELECT to_regclass('public.email_otps')")).scalar()
        location_count = db.execute(text("SELECT COUNT(*) FROM locations")).scalar()
        print(f"app_users table: {app_users}")
        print(f"email_otps table: {email_otps}")
